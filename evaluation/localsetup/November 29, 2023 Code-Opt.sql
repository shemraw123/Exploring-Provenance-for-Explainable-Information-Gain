...
--     CASE WHEN igdaysbit = sgdaysbit THEN '0' ELSE hammingDist(igdaysbit::text,'0000000000') END AS hammingdistDaysGdays,
...
removeNoGoodPatt AS (
    SELECT *
    FROM patterns1
    WHERE patternIG > 0 AND (coverage > 1 OR (coverage = 1 AND informativeness = 5))),

-- cleaning up patterns more where k = 3


-- patterns with coverage > 1 and informativeness < 5
topKPattConstPlac AS (
    SELECT *
...
fscoreTopK AS (
    ...
    FROM topKPattConstPlac
    ORDER BY fscoreTopK DESC
    LIMIT 11),


-- this calculates fscore for topK1 patterns
fscoreTopKOnlyConst AS (
    SELECT i_county, i_year, i_dayswaqi, i_gdays, i_maqi, patternIG, coverage, informativeness,
        (3 * (patternIG * coverage * informativeness) / (patternIG + coverage + informativeness)) AS fscoreTopK
    FROM topKPattOnlyConst
    ORDER BY fscoreTopK DESC
    --LIMIT 10
),


fscoreTopKOnlyConstSamp AS (
    SELECT l.*
    FROM fscoreTopKOnlyConst l, (SELECT MIN(fscoreTopK) AS mfscoreTopK FROM fscoreTopK) r
--     WHERE l.fscoreTopK > (SELECT MIN(fscoreTopK) FROM fscoreTopK)
    WHERE l.fscoreTopK > r.mfscoreTopK
    LIMIT 10),
...
analysisCorrelation as(
    SELECT i_county, i_year, i_dayswaqi, i_gdays, i_maqi, patternIG, coverage, informativeness,
        1+COALESCE(regr_r2(total_ig_rows, ig_county),0) as county_r2,
        1+COALESCE(regr_r2(total_ig_rows, ig_year),0) as year_r2,
        1+COALESCE(regr_r2(total_ig_rows, ig_dayswaqi),0) AS dayswaqi_r2,
        1+COALESCE(regr_r2(total_ig_rows, ig_gdays),0) AS gdays_r2,
        1+COALESCE(regr_r2(total_ig_rows, ig_maqi),0) AS maqi_r2
    FROM getData
