...
rowIG as(
    ...
    COUNT(IG_County) OVER() AS totalCnt,
    SUM(IG_County + IG_Year + IG_Maqi + IG_DaysWaqi + IG_Gdays) OVER() AS totalIG
	from hammingValue),

patterns1 as(
	SELECT i_county, i_year, i_dayswaqi, i_gdays, i_maqi,
	   (CASE WHEN MAX(TOTAL_IG_rows::float) != 0 THEN SUM(Total_IG_rows::float) / MAX(totalIG::float) ELSE 0 END) as patternIG, 
       (COUNT(*)::float / MAX(totalCnt)::float) AS coverage,
       (CASE WHEN (i_county IS NOT NULL) THEN 1 ELSE 0 END + 
        ...
        CASE WHEN (i_maqi IS NOT NULL) THEN 1 ELSE 0 END)::float / 5 as informativeness
	FROM rowIG
